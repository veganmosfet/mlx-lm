...
def stopping_criteria(
    ...
    eos_token_ids: Optional[Collection[int]],
) -> StopCondition:
    """
    ...
    Args:
        ...
        eos_token_ids (Optional[Collection[int]]): The token IDs that represent
          the end-of-sequence. If the last token in `tokens` matches any of
          these, the generation should stop.
    ...
    """
    if eos_token_ids and tokens and tokens[-1] in eos_token_ids:
...
class APIHandler(BaseHTTPRequestHandler):
    def __init__(
        ...
        **kwargs,
        ...
        self.enable_harmony_adapter=enable_harmony_adapter
    ...
    def generate_response(
        ...
        """
        Generate a single response packet based on completion type and parameters.
        ...
        Args:
            ...
            prompt_token_count (Optional[int]): The number of tokens in the prompt,
              used to populate the "usage" field .
            completion_token_count (Optional[int]): The number of tokens in the
              response, used to populate the "usage" field .
    ...
    def handle_completion(
        ...
        for gen_response in stream_generate(
            ...
            stop_condition = stopping_criteria(
                tokens, stop_id_sequences, self.tokenizer.eos_token_ids
            )
        ...
        if self.enable_harmony_adapter==True:
            parsed_response = self.harmony_encoding.parse_messages_from_completion_tokens(tokens, Role.ASSISTANT)
            logging.debug(parsed_response)
            for message in parsed_response:
                if message.channel=='final':
                    message_text+=message.content[0].text
                if message.channel=='analysis':
                    think_text+=message.content[0].text
                if message.channel=='commentary':
                    r = message.recipient
                    if r.startswith('functions.'):
                        r = r.removeprefix('functions.')
                        # model has problems and often adds additional token appended to function
                        r_clean = re.sub(r"<.*$", "", r)
                        tool_text='{"name": "'+r_clean+'", "arguments": '+message.content[0].text+'}'
                        tool_calls.append(tool_text)

        else:
            for token in tokens:
                if self.tokenizer.has_tool_calling and self.tokenizer.decode(token)==self.tokenizer.tool_call_start:
                    in_tool_call=True
                elif self.tokenizer.has_thinking and self.tokenizer.decode(token)==self.tokenizer.think_start:
                    in_thinking=True
                elif in_tool_call:
                    if self.tokenizer.decode(token) == self.tokenizer.tool_call_end:
                        tool_calls.append(tool_text)
                        tool_text = ""
                        in_tool_call = False
                    else:
                        tool_text += self.tokenizer.decode(token)
                elif in_thinking:
                    if self.tokenizer.decode(token) == self.tokenizer.think_end:
                        in_thinking = False
                    else:
                        thinking_text += self.tokenizer.decode(token)
                else:
                    if token not in self.tokenizer.eos_token_ids:
                        message_text += self.tokenizer.decode(token)
...
def main():
    ...
    run(args.enable_harmony_adapter ,args.host, args.port, ModelProvider(args))
